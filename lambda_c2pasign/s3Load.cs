                    ...
                    foreach (string currfile in FilesToUploadFinal)
                    {
                        ...
                        allTasks.Add(
                            Task.Run(async () =>
                            {
                                try
                                {
                                    string tmpKey = currfile.Replace(PathToRemove, "").Trim('/');
                                    var uploadRequest =
                                        new TransferUtilityUploadRequest
                                        {
                                            BucketName = bucket,
                                            
                                            Key = Path.Combine(s3BucketPath.Trim('/'), tmpKey).TrimStart('/'),
                                            FilePath = currfile
                                        };

                                    Console.WriteLine("DEBUG tmpKey upload " + tmpKey);
                                    Console.WriteLine("DEBUG Key upload " + Path.Combine(s3BucketPath.Trim('/'), tmpKey).TrimStart('/'));
